...
- **Avro schema resolution is now pinned to the same bytes in all five
  ...
  schema and so always have one. Nothing about the behaviour changed; it was
  already this, and now it is written down and asserted.
  ...
  **This library is in the `resolved` column and can reach both.** The fixture
  ...
  arguments say which schema to resolve *onto*, not whether to.
